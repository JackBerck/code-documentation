...
/*
-------- ABSTRACT CLASS ----------
Abstract class adalah class yang tidak dapat diinstansiasi menjadi objek, tetapi dapat dijadikan sebagai blueprint untuk class-class turunannya. Abstract class digunakan untuk mendefinisikan properti dan metode yang harus dimiliki oleh class turunannya, namun tidak memberikan implementasi yang sebenarnya.

Abstract class biasanya digunakan untuk membuat class-class yang memiliki kesamaan properti dan metode, namun memiliki implementasi yang berbeda. Abstract class memungkinkan Anda untuk mengelompokkan class-class yang memiliki kesamaan dan memaksa class turunan untuk mengimplementasikan metode-metode yang didefinisikan dalam abstract class.

Untuk mendefinisikan abstract class, Anda dapat menggunakan kata kunci abstract diikuti dengan nama class, dan untuk mendefinisikan metode abstrak, Anda dapat menggunakan kata kunci abstract diikuti dengan nama metode tanpa memberikan implementasi.

Berikut adalah contoh penggunaan abstract class dalam PHP:
...
echo $circle->getArea(); // Output: 314.15926535898
*/

/*
-------- INTERFACE ----------
Interface adalah kontrak yang digunakan untuk mendefinisikan metode tanpa memberikan implementasi. Interface hanya mendefinisikan metode yang harus dimiliki oleh class yang mengimplementasikan interface tersebut, namun tidak memberikan implementasi sebenarnya.

Interface digunakan untuk membuat class-class yang memiliki metode yang sama, namun memiliki implementasi yang berbeda. Dengan interface, Anda dapat membuat class-class yang memiliki kesamaan metode tanpa harus mewarisi dari class lain.

Ciri-ciri interface:
Interface hanya mendefinisikan metode tanpa memberikan implementasi.
Murni merupakan template untuk class turunannya.
Tidak boleh memiliki property, hanya metodhe saja.
Semua methodnya harus public.
Boleh mendeklarasikan construct.
Satu class bisa mengimplementasikan banyak interface.
Dengan menggunakan type hinting dapat menggunakan Dependency Injection.
Pada akhirnya akan mencapai Polymorphism.

Untuk mendefinisikan interface, Anda dapat menggunakan kata kunci interface diikuti dengan nama interface, dan untuk mendefinisikan metode dalam interface, Anda dapat menggunakan nama metode tanpa memberikan implementasi.
...
/*
-------- AUTOLOADING ----------
Autoloading adalah proses yang digunakan untuk memuat class secara otomatis tanpa harus memanggil fungsi require atau include secara manual. Autoloading memungkinkan Anda untuk memuat class secara dinamis saat class tersebut dibutuhkan, sehingga menghemat waktu dan usaha dalam pengelolaan file class.

Di dalam PHP, Anda dapat menggunakan fungsi spl_autoload_register() untuk mendaftarkan fungsi autoload yang akan dipanggil ketika class yang belum didefinisikan diakses. Fungsi autoload tersebut akan mencari dan memuat file class yang sesuai dengan nama class yang diminta.

Berikut adalah contoh penggunaan autoloading dalam PHP:
...
/*
-------- NAMESPACE ----------
Namespace adalah fitur dalam PHP yang digunakan untuk mengelompokkan class, interface, fungsi, dan konstanta ke dalam satu namespace tertentu. Namespace memungkinkan Anda untuk menghindari konflik nama antar class dan memudahkan dalam mengelola class-class yang ada.

Dengan menggunakan namespace, Anda dapat membuat class dengan nama yang sama tanpa harus khawatir terjadi konflik nama. Namespace juga memungkinkan Anda untuk mengelompokkan class-class yang memiliki kesamaan ke dalam satu namespace tertentu.

Untuk mendefinisikan namespace, Anda dapat menggunakan kata kunci namespace diikuti dengan nama namespace, dan untuk menggunakan class yang berada di dalam namespace, Anda dapat menggunakan kata kunci use diikuti dengan nama class yang diinginkan.

Penulisan namespace yang benar:
Namespace harus dideklarasikan di baris pertama file PHP.
Namespace harus dideklarasikan setelah tag <?php.
Namespace harus dideklarasikan sebelum class, interface, atau fungsi lainnya.
Namespace harus dideklarasikan sebelum menggunakan class, interface, atau fungsi lainnya.
Namespace harus dideklarasikan sebelum menggunakan konstanta.
Namespace harus dideklarasikan sebelum menggunakan trait.

Dan penulisan namespace yang dianjurkan adalah:
namespace Vendor\Namespace\SubNamespace;

Berikut adalah contoh penggunaan namespace dalam PHP:
// File: Shape.php
namespace Geometry;
